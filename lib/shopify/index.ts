...
export async function getMetaobject({
  ...
  return res.body.data.metaobject.fields.reduce<{ [key: string]: string }>((acc, {jsonValue, key}) => {
    acc[key] = jsonValue;
    return acc;
  }, {});
}
...
export async function createNewsletterSubscriber(
  ...
  try {
    const input: CustomerCreateInput = {
      ...
      password: process.env.CUSTOMER_DEFAULT_PASSWORD || 'voircurstomeraccount123',
