...
const reshapeProduct = (
  ...
  // It's good practice to add the return type
  if (
    !product ||
  ...
  return {
    ...
    // --- Add this new metafield mapping ---
    video: product.video?.value || null,
    careInstructions: product.careInstructions?.value || null,
    materials: product.materials?.value || null,
    sizeGuide: product.sizeGuide?.value || null,
  };
};
...
export async function getMetaobject({
  ...
  return res.body.data.metaobject.fields.reduce<{ [key: string]: string }>(
    (acc, { jsonValue, key }) => {
      acc[key] = jsonValue;
      return acc;
    },
...
export async function createNewsletterSubscriber(
  ...
  try {
    const input: CustomerCreateInput = {
      ...
      password:
        process.env.CUSTOMER_DEFAULT_PASSWORD || "voircurstomeraccount123",
      // Explicitly skip password
    };
    ...
    if (res.body.data.customerCreate.userErrors?.length) {
      return {
        errors: res.body.data.customerCreate.userErrors,
      };
    }
